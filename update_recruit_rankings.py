...
def repair_existing_name(name, school):
    """Remove an accidentally appended school suffix from an existing player name."""
    ...
    # Try all possible right-side suffix lengths and remove the longest suffix
    # that canonicalizes exactly to the player's school.
    for n in range(min(len(tokens)-1, 6), 0, -1):
        suffix = " ".join(tokens[-n:])
        if canon(suffix) == school_id:
...
def main():
    ...
    # Final cleanup/sanity pass.
    cleaned = []
    for p in players:
        p["name"] = repair_existing_name(p.get("name"), p.get("school"))
        if not p.get("name") or norm(p["name"]) == canon(p.get("school")):
            continue
        cleaned.append(p)
    players = cleaned
