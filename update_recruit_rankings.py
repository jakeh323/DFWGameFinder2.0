...
def parse_commitment_page(college, slug, year, schools):
    ...
    for txt in candidates:
        ...
        # Identify school using the statewide game index. Match longest exact
        # canonical school phrase first so North Crowley != Crowley.
        school_display = None
        school_canon = None
        left_norm = canon(strip_mascot(left))
        for ck, display in schools:
            if left_norm.endswith(ck):
                school_canon = ck
                school_display = display
                break
            # Also accept an explicit alias canonical match at the end.
            if canon(left_norm).endswith(ck):
                school_canon = ck
                school_display = display
                break
        ...
        # Recover player name by stripping the school phrase (and mascot) from the right.
        raw_school_variants = [school_display, strip_mascot(school_display)]
        name = left
        # Use normalized token length to split safely.
        left_tokens = norm(left).split()
        school_tokens = school_canon.split()
        if len(left_tokens) > len(school_tokens):
            name_tokens = left_tokens[:-len(school_tokens)]
            # Normalization loses capitalization; recover name from existing DB later when possible.
            guessed = " ".join(name_tokens)
            name = guessed.title()
        else:
            continue
...
def main():
    ...
    print(f"Updated {len(players)} prospects; {len(fresh)} commitment records were parsed.")
